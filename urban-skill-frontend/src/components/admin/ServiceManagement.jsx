...
const Reports = () => {
  ...
  const reportData = {
    ...
    monthlyData: [
      { month: 'Jan', revenue: 185000, bookings: 1420, users: 2100, workers: 380, rating: 4.6 },
      { month: 'Feb', revenue: 223000, bookings: 1510, users: 2350, workers: 420, rating: 4.7 },
      { month: 'Mar', revenue: 289000, bookings: 1680, users: 2800, workers: 480, rating: 4.8 },
      { month: 'Apr', revenue: 312000, bookings: 1750, users: 3100, workers: 520, rating: 4.7 },
      { month: 'May', revenue: 358000, bookings: 1820, users: 3400, workers: 580, rating: 4.8 },
      { month: 'Jun', revenue: 421000, bookings: 1950, users: 3850, workers: 640, rating: 4.9 }
    ],
    servicePerformance: [
      { service: 'House Cleaning', bookings: 3200, revenue: 480000, avgRating: 4.8, workers: 850 },
      { service: 'Plumbing', bookings: 2100, revenue: 315000, avgRating: 4.7, workers: 520 },
      { service: 'Electrical', bookings: 1800, revenue: 360000, avgRating: 4.6, workers: 420 },
      { service: 'Painting', bookings: 1200, revenue: 240000, avgRating: 4.9, workers: 280 },
      { service: 'Appliance Repair', bookings: 650, revenue: 130000, avgRating: 4.5, workers: 180 }
    ],
    workerPerformance: [
      { name: 'John Smith', bookings: 156, revenue: 234000, rating: 4.9, completion: 98 },
      { name: 'Sarah Johnson', bookings: 142, revenue: 213000, rating: 4.8, completion: 96 },
      { name: 'Mike Wilson', bookings: 138, revenue: 207000, rating: 4.7, completion: 95 },
      { name: 'Emma Davis', bookings: 134, revenue: 201000, rating: 4.9, completion: 97 },
      { name: 'David Brown', bookings: 129, revenue: 193500, rating: 4.6, completion: 94 }
    ],
    customerAnalytics: [
      { segment: 'New Customers', count: 4200, revenue: 315000, avgSpend: 75 },
      { segment: 'Returning Customers', count: 8950, revenue: 671250, avgSpend: 75 },
      { segment: 'Premium Customers', count: 2270, revenue: 340500, avgSpend: 150 }
    ],
    geographicData: [
          ...
          <>
            ...
            <Grid container spacing={3} mb={4}>
              <Grid item xs={12} sm={6} md={3}>
                <StatCard
                  title="Total Revenue"
                  value={`₹${(reportData.overview.totalRevenue / 100000).toFixed(1)}L`}
                    ...
                    <ResponsiveContainer width="100%" height={350}>
                      <RechartsPieChart>
                        <Pie
                          ...
                          outerRadius={100}
                          dataKey="bookings"
                          label={({ service, bookings }) => `${service}: ${bookings}`}
                        >
                          {reportData.servicePerformance.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={['#6366f1', '#f59e0b', '#10b981', '#ef4444', '#8b5cf6'][index]} />
                          ))}
                        </Pie>
                        <RechartsTooltip />
                      </RechartsPieChart>
                        ...
                        <TableRow key={service.service} hover>
                          ...
                          <TableCell align="center">
                            <Chip
                              ...
                              size="small"
                              sx={{
                                backgroundColor: service.avgRating >= 4.7 ? '#10b981' : service.avgRating >= 4.5 ? '#f59e0b' : '#ef4444',
                                color: 'white',
                                fontWeight: 'bold'
                              }}
                            />
                          ...
                          <TableCell align="center">
                            <LinearProgress
                              variant="determinate"
                              value={(service.bookings / Math.max(...reportData.servicePerformance.map(s => s.bookings))) * 100}
                              sx={{
                                height: 8,
                                borderRadius: 4,
                ...
                <CardContent>
                  ...
                  <Box display="flex" flexDirection="column" gap={2}>
                    {reportData.customerAnalytics.map((segment) => (
                      <Box key={segment.segment} p={2} sx={{ backgroundColor: '#f8fafc', borderRadius: 2 }}>
                ...
                <CardContent>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={reportData.monthlyData}>
                      ...
                      <RechartsTooltip />
                      <Legend />
                      <Line type="monotone" dataKey="users" stroke="#6366f1" strokeWidth={3} name="New Users" />
                      <Line type="monotone" dataKey="workers" stroke="#f59e0b" strokeWidth={3} name="New Workers" />
                      <Line type="monotone" dataKey="rating" stroke="#10b981" strokeWidth={3} name="Avg Rating" />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  ...
                  <Box display="flex" flexDirection="column" gap={2}>
                    {reportData.geographicData.map((city, index) => (
                      <Box key={city.city} p={2} sx={{ backgroundColor: '#f8fafc', borderRadius: 2 }}>
                        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                          <Box display="flex" alignItems="center" gap={2}>
                            <Chip
                              label={index + 1}
